...
public class MyWaveProgress extends View {

    private static final String TAG = MyWaveProgress.class.getSimpleName();
...
//    private float mSuffixTextSize;
    ...
    public MyWaveProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MyWaveProgress);

        antiAlias = typedArray.getBoolean(R.styleable.MyWaveProgress_antiAlias, true);
        mDarkWaveAnimTime = typedArray.getInt(R.styleable.MyWaveProgress_darkWaveAnimTime, Constant.DEFAULT_ANIM_TIME);
        mLightWaveAnimTime = typedArray.getInt(R.styleable.MyWaveProgress_lightWaveAnimTime, Constant.DEFAULT_ANIM_TIME);
        mMaxValue = typedArray.getFloat(R.styleable.MyWaveProgress_maxValue, Constant.DEFAULT_MAX_VALUE);
        mMinValue = typedArray.getFloat(R.styleable.MyWaveProgress_minValue, Constant.DEFAULT_MIN_VALUE);
        mValue = typedArray.getFloat(R.styleable.MyWaveProgress_value, Constant.DEFAULT_VALUE);
        mValueSize = typedArray.getDimension(R.styleable.MyWaveProgress_valueSize, Constant.DEFAULT_VALUE_SIZE);
        mValueColor = typedArray.getColor(R.styleable.MyWaveProgress_valueColor, Color.BLACK);
        // TODO 提示文字
        mHint = typedArray.getString(R.styleable.MyWaveProgress_hint);
        mHintColor = typedArray.getColor(R.styleable.MyWaveProgress_hintColor, Color.BLACK);
        mHintSize = typedArray.getDimension(R.styleable.MyWaveProgress_hintSize, Constant.DEFAULT_HINT_SIZE);
        // TODO 后缀文字
        String suffixText = typedArray.getString(R.styleable.MyWaveProgress_suffixText);
        mSuffixText = suffixText != null ? suffixText : "%";
//        mSuffixTextColor = typedArray.getColor(R.styleable.WaveProgress_suffixTextColor, Color.BLACK);
//        mSuffixTextSize = typedArray.getDimension(R.styleable.WaveProgress_suffixTextSize, Constant.DEFAULT_HINT_SIZE);
        // TODO 圆环
        mCircleWidth = typedArray.getDimension(R.styleable.MyWaveProgress_circleWidth, Constant.DEFAULT_ARC_WIDTH);
        mCircleColor = typedArray.getColor(R.styleable.MyWaveProgress_circleColor, Color.GREEN);
        mBgCircleColor = typedArray.getColor(R.styleable.MyWaveProgress_bgCircleColor, Color.WHITE);
        // TODO 水波纹
        mWaveHeight = typedArray.getDimension(R.styleable.MyWaveProgress_waveHeight, Constant.DEFAULT_WAVE_HEIGHT);
        mWaveNum = typedArray.getInt(R.styleable.MyWaveProgress_waveNum, 1);
        mDarkWaveColor = typedArray.getColor(R.styleable.MyWaveProgress_darkWaveColor,
                ContextCompat.getColor(context, android.R.color.holo_blue_dark));
        mLightWaveColor = typedArray.getColor(R.styleable.MyWaveProgress_lightWaveColor,
                ContextCompat.getColor(context, android.R.color.holo_green_light));

        isR2L = typedArray.getInt(R.styleable.MyWaveProgress_lightWaveDirect, R2L) == R2L;
        lockWave = typedArray.getBoolean(R.styleable.MyWaveProgress_lockWave, false);
...
//        mSuffixTextPaint.setAntiAlias(antiAlias);
...
//        }
        if (mPrePercent == 0.0f || Math.abs(mPercent - mPrePercent) >= 0.01f) {
            ...
            value = String.format(Locale.getDefault(), "%.0f %s", mValue, mSuffixText);
        }
        canvas.drawText(value, mCenterPoint.x, y, mPercentPaint);
...
//        if (mSuffixText != null) {
//            mSuffixTextPaint.setTextAlign(Paint.Align.CENTER);
//            float hx = mCenterPoint.x + mPercentPaint.measureText(mPercentValue) / 2;
//            float hy = mCenterPoint.y - (mSuffixTextPaint.descent() + mSuffixTextPaint.ascent()) / 2;
//            canvas.drawText(mSuffixText.toString(), hx, y, mSuffixTextPaint);
//        }
    }
